...
app.post('/api/analyze', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    let healthAnalysis;
    const llmResult = await getLLMAnalysis(nutrition, dvPercent, ingredients);
    if (llmResult.analysis) {
      healthAnalysis = llmResult.analysis;
    } else {
      healthAnalysis = ruleBasedFallback(dvPercent) + (llmResult.error ? ` (LLM unavailable: ${llmResult.error})` : '');
    }
