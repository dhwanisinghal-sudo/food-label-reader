...
| Export to JSON/PDF | ✅ | ✅ | ✅ |
| Scan history | SQLite, local to that notebook session only | MongoDB, saved per logged-in account | Reads the same MongoDB history as the web app |
| Accounts / login | ❌ *(the notebook is a single-session script with no concept of separate users, not an app — this is a design difference, not an unfinished feature)* | ✅ (email + password, JWT) | ✅ (shares the web app's accounts) |
...
2. Runtime → Run all
3. Upload one or more nutrition label photos (Ctrl/Cmd+click for multiple)
