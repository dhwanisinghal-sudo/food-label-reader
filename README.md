...
| Multi-image upload, one photo at a time processed | ✅ | ✅ | ➖ (not built) |
| Nutrition parsing (calories, fat, sodium, carbs, sugar, protein, fiber, cholesterol, vitamins/minerals) | ✅ | ✅ | ✅ |
| Health score (0–100, custom heuristic) | ✅ | ✅ | ✅ |
| **FSA/Ofcom Nutrient Profiling Model score** (published, government-adopted model) | ✅ | ➖ | ➖ |
...
| Export to JSON/PDF | ✅ | Partial (JSON via API) | ❌ |
| Scan history | SQLite (local, notebook session only) | MongoDB (per logged-in account) | Reads the same MongoDB history |
