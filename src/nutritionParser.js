...
// FIX (real gap found, mirrors nutrition_parser.py's parse_serving_size):
// a label like "Serving Size 1 Bag" used to be stored as an opaque raw
// string with no gram amount extractable at all -- no way to compare it
// against OpenFoodFacts' per-100g values. Returns null if no serving-size
// line was found; otherwise { raw, amountG, householdMeasure, ambiguous }.
// ambiguous=true means no gram-equivalent could be extracted -- callers
// must NOT assume a default gram amount when this is true.
function parseServingSize(text) {
  let m = text.match(/serving size\s*([^\n]+)/i);
  if (!m) {
    // Non-US ("Canadian") labels often say "Per 2 slices (64 g)" instead of
    // "Serving Size ...". Anchored to the start of a line so this doesn't
    // accidentally match "servings PER container" appearing mid-line.
    m = text.match(/^\s*per\s+([^\n]+)/im);
  }
  if (!m) return null;
  const raw = m[1].trim();

  let amountG = null;
  const parenMatch = raw.match(/\(\s*([0-9OoIl]+\.?[0-9]*)\s*[g)93]\s*[/)]/i);
  if (parenMatch) {
    const val = cleanNum(parenMatch[1]);
    if (val !== null && val > 0 && val <= 1000) amountG = val;
  }
  if (amountG === null) {
    const directMatch = raw.match(/^\s*([0-9OoIl]+\.?[0-9]*)\s*g\b/i);
    if (directMatch) {
  ...
  const householdMeasure = householdMatch ? householdMatch[1].trim().replace(/\/$/, '').trim() || null : null;

  return { raw, amountG, householdMeasure, ambiguous: amountG === null };
}
...
const servingSizeParsed = parseServingSize(text);
if (servingSizeParsed) {
  data.serving_size = servingSizeParsed.raw;
  data.serving_size_parsed = servingSizeParsed;
}
