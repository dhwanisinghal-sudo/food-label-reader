...
Flagged as low OCR confidence (<60 avg): 6
...
- Overall field accuracy is 60.0%, but resampling the photos gives a 95% bootstrap interval of **42.6% - 76.6%**. Fields within one photo are correlated (a blurry photo fails on almost every field at once), so the effective sample is closer to the number of photos than to the number of fields.
...
## Failure cases: 63 logged -> see failure_cases.csv
